...
_libc.scaling.argtypes = [c_int, ndpointer(c_float), c_int, c_int, c_int]

_libf = CDLL(os.path.join(_libdir, "libgcalcf.so"))
_libf.scaling_.restype = None
_libf.scaling_.argtypes = [POINTER(c_int), ndpointer(c_float), POINTER(c_int), POINTER(c_int), POINTER(c_int)]
...
def scaling_c(num, arr):
    ...
    ny = arr.shape[1]
    nz = arr.shape[0]
    _libc.scaling(num, arr, nx, ny, nz)

def scaling_f(num, arr):
    nx = arr.shape[2]
    ny = arr.shape[1]
    nz = arr.shape[0]
    _libf.scaling_(byref(c_int(num)), arr, byref(c_int(nx)), byref(c_int(ny)), byref(c_int(nz)))
